...
2. Derive a friendly name (`pyvenv.cfg` `prompt`, else parent dir) and a
   collision-resistant identifier `ai.mxs.tcc.<name>.<sha256(realpath)[:8]>`.
...
- `posix_spawn`s `<venv>/bin/python` (falls back to `python3`) and stays the live
  ...
  and loses the grant).
- forwards SIGINT/TERM/HUP/QUIT/USR1/USR2 to the child, propagates exit status
  (`128 + signo` on signal death).
